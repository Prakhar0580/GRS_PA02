...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <msg_size> <duration_sec>\n", argv[0]);
        return 1;
    }
    ...
    while (time(NULL) < end_test) {
        int bytes = recv(sock, buffer, msg_size, 0);
        if (bytes <= 0) break;
        if (strncmp(buffer, "REJECTED", 8) == 0) goto end_prog;
        total_bytes += bytes;
    }
    ...
    double gbps = (total_bytes * 8.0) / (sec * 1e9);
    double total_msgs = (double)total_bytes / msg_size;
    double latency = (total_msgs > 0) ? (sec * 1e6) / total_msgs : 0;

    // Output strictly for the Bash Script: Throughput,Latency
    printf("%.4f,%.2f\n", gbps, latency);
