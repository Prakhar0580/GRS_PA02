...
void *handle_client(void *socket_desc) {
    int sock = *(int*)socket_desc;
    int field_size = 4096 / 8; // Adjust based on your experiment needs
    ...
    while (1) {
        if (transmission_mode == 1) { // 2-Copy (Standard Send)
            for (int i = 0; i < 8; i++) {
                if (send(sock, fields[i], field_size, 0) <= 0) goto cleanup;
            }
        } 
        else { // Mode 2 (1-copy sendmsg) or Mode 3 (Zero-copy)
            struct msghdr msg = {0};
            struct iovec iov[8];
            for (int i = 0; i < 8; i++) {
            ...
            msg.msg_iov = iov;
            msg.msg_iovlen = 8;
            int flags = (transmission_mode == 3) ? 0x4000000 : 0; // MSG_ZEROCOPY flag
            if (sendmsg(sock, &msg, flags) <= 0) goto cleanup;
        }
    }

cleanup:
    for (int i = 0; i < 8; i++) free(fields[i]);
    close(sock);
    free(socket_desc);
    active_clients--;
    return NULL;
